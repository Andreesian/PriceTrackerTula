...
def add_user(connection: psycopg2.extensions.connection, id_user: int, nickname: str, request_ids: Optional[List[int]] = None):
    ...
    with connection.cursor() as cursor:
        cursor.execute("INSERT INTO users (id_user, nickname, request_id) VALUES (%s, %s, %s)", (id_user, nickname, request_ids))
        connection.commit()
...
def add_request(connection: psycopg2.extensions.connection, id_request: int, product_name: str, update_time: str, price_history: List[int], url_id: int):
    with connection.cursor() as cursor:
        cursor.execute("INSERT INTO requests (id_request, product_name, update_time, price_history, url_id) VALUES (%s, %s, %s, %s, %s)",
...
def add_url(connection: psycopg2.extensions.connection, id_url: int, url: str, short_name: str):
    with connection.cursor() as cursor:
        cursor.execute("INSERT INTO urls (id_url, url, short_name) VALUES (%s, %s, %s)", (id_url, url, short_name))
        connection.commit()

def get_user_by_id(connection: psycopg2.extensions.connection, id_user: int) -> Optional[Tuple[int, str, List[int]]]:
    with connection.cursor() as cursor:
        cursor.execute("SELECT id_user, nickname, request_id FROM users WHERE id_user = %s", (id_user,))
...
def get_request_by_id(connection: psycopg2.extensions.connection, id_request: int) -> Optional[Tuple[int, str, str, List[int], int]]:
    with connection.cursor() as cursor:
        cursor.execute("SELECT id_request, product_name, update_time, price_history, url_id FROM requests WHERE id_request = %s", (id_request,))
        result = cursor.fetchone()
    return result

def get_url_by_id(connection: psycopg2.extensions.connection, id_url: int) -> Optional[Tuple[int, str, str]]:
    with connection.cursor() as cursor:
        cursor.execute("SELECT id_url, url, short_name FROM urls WHERE id_url = %s", (id_url,))
        result = cursor.fetchone()
    return result
...
def update_user(connection: psycopg2.extensions.connection, id_user: int, new_nickname: Optional[str] = None, new_request_ids: Optional[List[int]] = None):
    with connection.cursor() as cursor:
        if new_nickname:
            cursor.execute("UPDATE users SET nickname = %s WHERE id_user = %s", (new_nickname, id_user))
        if new_request_ids is not None:
            cursor.execute("UPDATE users SET request_id = %s WHERE id_user = %s", (new_request_ids, id_user))
        connection.commit()
...
def update_request(connection: psycopg2.extensions.connection, id_request: int, new_product_name: Optional[str] = None, new_update_time: Optional[str] = None,
    ...
    with connection.cursor() as cursor:
        if new_product_name:
            cursor.execute("UPDATE requests SET product_name = %s WHERE id_request = %s", (new_product_name, id_request))
        if new_update_time:
            cursor.execute("UPDATE requests SET update_time = %s WHERE id_request = %s", (new_update_time, id_request))
        if new_price_history is not None:
            cursor.execute("UPDATE requests SET price_history = %s WHERE id_request = %s", (new_price_history, id_request))
        if new_url_id:
            cursor.execute("UPDATE requests SET url_id = %s WHERE id_request = %s", (new_url_id, id_request))
        connection.commit()
...
def update_url(connection: psycopg2.extensions.connection, id_url: int, new_url: Optional[str] = None, new_short_name: Optional[str] = None):
    with connection.cursor() as cursor:
        if new_url:
            cursor.execute("UPDATE urls SET url = %s WHERE id_url = %s", (new_url, id_url))
        if new_short_name:
            cursor.execute("UPDATE urls SET short_name = %s WHERE id_url = %s", (new_short_name, id_url))
        connection.commit()
...
def delete_request(connection: psycopg2.extensions.connection, id_request: int):
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM requests WHERE id_request = %s", (id_request,))
        connection.commit()

def delete_url(connection: psycopg2.extensions.connection, id_url: int):
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM urls WHERE id_url = %s", (id_url,))
        connection.commit()

# Example usage
connection = create_connection("your_database", "your_user", "your_password", "your_host", "your_port")
...
update_user(connection, 1, "JaneDoe")
